...
export default function BookQueue({ route }) {
    ...
    const [userId, setUserId] = useState("12312312312312 54545345345345345");


    useEffect(() => {
        const fetchDeviceId = async () => {
            // Try to get the stored device ID from AsyncStorage
            const storedDeviceId = await AsyncStorage.getItem('deviceId');

            if (storedDeviceId) {
                setUserId(storedDeviceId); // Use the stored device ID
            } else {
                // Get unique ID from the device
                const uniqueDeviceId = Device.getUniqueId();

                // Store the generated ID in AsyncStorage for future use
                await AsyncStorage.setItem('deviceId', uniqueDeviceId);
    ...
    const book_new_queue = async () => {
        try {
            setLoading(true)
            await axios.post(`${info.appUrl}/api/v1/queues/book/new/queue/${userId}/${place._id}/${service._id}`)
            setLoading(false)
        } catch (error) {
            console.log(error)
            setLoading(false)
